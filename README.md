...
| **IAM self-inspection** (`iam:GetRole`, `iam:GetRolePolicy`, `iam:ListAttachedRolePolicies`, `iam:ListRolePolicies`, `iam:ListRoleTags`, `iam:SimulatePrincipalPolicy` scoped to `role/cloudzero/*`; `iam:GetPolicy`, `iam:GetPolicyVersion` on attached managed policies) | CloudZero's own cross-account role and the policy documents attached to it | Transparency: CloudZero can show you exactly what access has been granted, which managed policies are attached, and the contents of those policies. Useful during audits and permission reviews. Scoped to CloudZero's own role path; `iam:GetPolicy`/`GetPolicyVersion` cannot be scoped to a specific role's attached policies by the IAM policy language, so it applies to policy documents generally. |
