...
| **Container Cost Tracking** | CloudWatch Logs (Container Insights), ECS, EKS | Attribute costs to containers, pods, and Kubernetes services; correlate Split Cost Allocation Data (SCAD) lines back to clusters, nodegroups, services, and task definitions | Container metrics, pod-level resource usage, cluster/nodegroup/Fargate-profile metadata, ECS service and task-definition configuration |
...
| **EKS / ECS describe and list** (`eks:Describe*/List*`, `ecs:Describe*/List*`) | Cluster names, nodegroup and Fargate-profile mapping, ECS services and task definitions | The `ViewOnlyAccess` managed policy covers a baseline of these APIs but does not include newer additions (EKS nodegroups, Fargate profiles, addons; ECS Service Connect and capacity-provider APIs). Required so CloudZero can correlate AWS Split Cost Allocation Data (SCAD) CUR lines and Container Insights metrics back to the actual cluster, nodegroup, service, and task definition that incurred the cost. Read-only — no mutations. |
